...
export default function Main({
  ...
  useEffect(() => {
    ...
    try {
      let newprovider;
      if (false /*window.location.hostname === "stage.ether.town"*/) {
        console.log(" 📡 using PUBLIC RPC provider for stage.ether.town");
        newprovider = new ethers.providers.JsonRpcProvider("https://chain.buidlguidl.com:8545");
      } else {
  ...
  const doContractDeployment = async () => {
    ...
    if(isDeploying){
      console.log("already deploying... wait....")
      return;
    }
    ...
    let deployerAccount = new ethers.Wallet("0x8a940e88f6dd4e97835a011b013b583b5294474f5513b6c823788ee40c0a1d83", localhostProvider);

    console.log("deploying as ",deployerAccount.address)

    let deployer = new ethers.ContractFactory(Level1.abi, Level1.bytecode, deployerAccount);
    ...
    if (result && result.address) {
      ...
      const contract = new ethers.Contract(result.address, Level1.abi, deployerAccount);
