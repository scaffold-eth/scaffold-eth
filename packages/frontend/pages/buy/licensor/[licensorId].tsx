...
function Licensor() : JSX.Element {
    ...
    const { brandId, brandName, brandSymbol, contractAddress, licenses } = licensorInfo || {}
    ...
    const selectNFT = (id) => {
        if (id) {
            const _selectedNFT = licenses.find(item => item.id === id)
            if (_selectedNFT) setSelectedNFT(_selectedNFT)
        }
    }
...
const LicenseItem = ({ item, selectNFT, isModalOpen, setIsModalOpen }) => {
