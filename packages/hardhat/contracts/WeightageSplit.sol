...
// File @uniswap/v3-periphery/contracts/libraries/TransferHelper.sol@v1.3.0
...
contract PublicGoodToken is ERC20 {
    ...
    mapping(address =>  mapping (address => uint)) public stakeAmount;
    ...
    /**
     * @notice Gets the RetroPGF price
     */
    function getPrice()
        external
        view
        returns (uint256 price)
    {
        ...
        if (getToken1() == address(this)) {          
          return 2 ** 192 / uint(sqrtPriceX96) ** 2;
        } else {
    ...
    function fundProject() external payable {
        ...
        if (positionManager.balanceOf(address(this)) > 0) {
            ...
            positionManager.increaseLiquidity{ value: msg.value }(params);
        } else {
            // mint a new position
            ...
            (uint mintedId, , , ) = positionManager.mint{ value: msg.value }(params);
            tokenId = mintedId;
        }
    }
...
function stake(uint _amount, PublicGoodToken[] memory _publicGoods) external {
   ...
   for (uint i = 0; i < _publicGoods.length; i++) {
       uint percentage = 100;
       if (_publicGoods.length > 1) {
       percentage = (_publicGoods[i].getPrice() * 100) / sum;
