...
/* ========== MUTATIVE FUNCTIONS ========== */
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        ...
        // require(token.approve(address(this), tokens), "DEX: init - Need to approve the movement of tokens");
        // emit Approved(msg.sender, address(this), tokens);
 ...
 */
     ...
     * So we are going to accept ETH, calculate the amount of tokens to obtain, then transfer them from the msg.sender to the DEX contract. We will keep track of the liquidity provided from the user as well instead of minting them an LPtoken or something.
     * Recall: Ratio needs to be maintained.
     */
     ...
     function deposit() public payable returns (uint256 tokensDeposited) {
         ...
         return tokenDeposit;
         emit LiquidityProvided(msg.sender, msg.value, tokenDeposit, liquidity[msg.sender], liquidityMinted, totalLiquidity);
     }
     ...
     */
     function withdraw(uint256 amount) public returns (uint256 eth_amount, uint256 token_amount) {
        ...
        ethWithdrawn =  amount.mul((ethReserve / totalLiquidity)); //why totalLiquidity, and not tokenReserve?
        ...
        // require(token.transferFrom(address(this), msg.sender, tokenAmount));

        return (ethWithdrawn, tokenAmount);
        emit LiquidityRemoved(msg.sender, ethWithdrawn, tokenAmount, liquidity[msg.sender], ethWithdrawn, totalLiquidity);
     }
